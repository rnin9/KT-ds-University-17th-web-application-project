...
<body>
	...
	<div class="container">
		...
		<div class="pageIntro" style="margin-bottom: 30px;">1:1 문의</div>
		...
		<div class="shadow p-3 mb-5 bg-body rounded">
			...
			<div id="question">
				...
				<button type="button" class="btn btn-outline-danger"
					onclick="location.href='${contextPath}/question/adminDeleteQuestion.do?questionNum=${readQuestion.questionNum}'">비공개</button>
				<button type="button" class="btn btn-outline-danger"
					onclick="location.href='${contextPath}/question/adminShowQuestion.do?questionNum=${readQuestion.questionNum}'">공개</button>
				<button type="button" class="btn btn-outline-danger"
					onclick="location.href='${contextPath}/question/listQuestion.do'">목록</button>
				<br> <br>
				...
				<div class="mb-3 row">
					...
					<div class="col-sm-10">
						<input type="text" readonly class="form-control-plaintext"
							id="question_del_YN" name="question_del_YN"
							value="<c:if test="${readQuestion.question_del_YN == 'Y'}">비공개</c:if><c:if test="${readQuestion.question_del_YN == 'N'}">공개</c:if>"
							readonly>
					</div>
				</div>
				...
				<div class="mb-3 row" id="questionContent">
					...
					<div class="col-sm-10">
						<textarea class="form-control-plaintext" id="questionContent"
							name="questionContent" required placeholder="질문 내용을 입력해주세요"
							readonly><c:out
    ...
    if (txtArea) {
		...
		<div class="shadow p-3 mb-5 bg-body rounded">
			...
			<form role="form" name="addComment" id="addComment" method="post"
				action="${contextPath}/question/addComment.do">
				...
				<div class="mb-3 row" id="questionCommentRegDate">
					<label for="questionCommentRegDate" class="col-sm-2 col-form-label">답변
						완료일</label>
					<div class="col-sm-10">
						<input type="text" class="form-control-plaintext" disabled
							id="questionCommentRegDate" name="questionCommentRegDate"
							value="<c:choose><c:when test="${readQuestion.questionCommentRegDate == null}">답변 미등록</c:when><c:when test="${readQuestion.questionCommentRegDate != null}"><c:out value="${readQuestion.questionCommentRegDate}"/></c:when></c:choose>"
							readonly />
					</div>
				</div>
				<div class="mb-3 row" id="questionCommentTitle">
					<label for="questionCommentTitle" class="col-sm-2 col-form-label">답변
						제목</label>
					<div class="col-sm-10">
						<input type="text" class="form-control" id="questionCommentTitle"
							name="questionCommentTitle" placeholder="답변 제목을 입력해주세요"
							value="${readQuestion.questionCommentTitle}" required />
					</div>
				</div>
				...
				<div class="mb-3 row" id="questionCommentContent">
					...
					<div class="col-sm-10">
						<textarea class="form-control" id="questionCommentContent"
							name="questionCommentContent" rows="9" required
							placeholder="답변 내용을 입력해주세요" required><c:out
